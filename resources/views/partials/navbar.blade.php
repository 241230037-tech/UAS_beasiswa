<nav class="bg-background border-b border-border sticky top-0 z-50 shadow-lg">
    <div class="max-w-screen-xl mx-auto px-4 flex items-center h-14 gap-3">
        <a href="{{ url('/home') }}" class="flex items-center gap-1 mr-2 shrink-0 hover:opacity-90 transition-opacity">
            <i data-lucide="graduation-cap" class="w-6 h-6 text-[#e53935]"></i>
            <span class="text-lg font-black text-foreground tracking-tight">
                PORTAL<span class="text-[#e53935]">BEASISWA</span>
            </span>
        </a>
        ...
        <form class="flex-1 flex items-center justify-center" id="navbar-search-form" action="{{ route('library') }}" method="GET">
            <div class="relative w-full max-w-md">
                <input type="text" name="q" id="navbar-search-input" placeholder="Cari beasiswa..."
                    value="{{ request('q') }}"
                    class="navbar-search-input w-full px-4 py-2 pr-10 rounded-full text-sm">
                <button type="submit" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div class="flex items-center gap-0.5 ml-auto shrink-0">
            <button type="button" id="btn-chatbot" class="p-2 text-muted-foreground hover:text-foreground transition-colors" title="Chatbot Pemandu">
                <i data-lucide="message-square" class="w-5 h-5"></i>
            </button>
            <button type="button" id="btn-theme" class="p-2 text-muted-foreground hover:text-foreground transition-colors" title="Mode Terang">
                <i data-lucide="sun" class="w-5 h-5" id="theme-icon"></i>
            </button>

            <div id="navbar-logged-in" class="hidden sm:flex items-center gap-2 ml-2">
                <div id="navbar-avatar" class="w-8 h-8 rounded-full border-2 border-[#e53935] overflow-hidden bg-muted cursor-pointer flex items-center justify-center hover:scale-105 active:scale-95 transition-transform" title="Pengaturan Akun">
                    <span id="navbar-avatar-letter" class="text-xs font-bold text-[#e53935]">U</span>
                    <img id="navbar-avatar-img" src="" alt="avatar" class="w-full h-full object-cover hidden">
                </div>
                <span id="navbar-username" class="text-xs text-muted-foreground truncate max-w-[100px]">User</span>
            </div>

            <a href="{{ route('login') }}" id="navbar-login-btn" class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-white bg-[#e53935] hover:bg-[#c62828] rounded-full transition-colors ml-1">
                <i data-lucide="log-in" class="w-4 h-4"></i>
                Login
            </a>

            <button type="button" id="btn-menu" class="p-2 text-muted-foreground hover:text-foreground transition-colors ml-1" title="Menu">
    ...
    <nav class="py-2 shrink-0">
        ...
        @foreach($menuItems as $item)
            ...
            <a href="{{ url($item['path']) }}"
                ...
                class="w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors {{ $active ? 'bg-[#e53935]/10 text-[#e53935] border-r-2 border-[#e53935] font-semibold' : 'text-muted-foreground hover:bg-muted hover:text-foreground' }}">
    ...
    <div class="flex-1 overflow-y-auto">
        <div id="sidebar-account-logged" class="hidden p-4 border-t border-border bg-muted/10 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3">
                <div id="sidebar-avatar" class="w-12 h-12 rounded-full border-2 border-[#e53935] overflow-hidden bg-muted flex items-center justify-center shrink-0 shadow-[2px_2px_0px_0px_rgba(229,57,53,0.2)]">
                    <span id="sidebar-avatar-letter" class="text-sm font-bold text-[#e53935]">U</span>
...
<div id="chatbot-panel" class="fixed top-14 right-0 w-80 sm:w-96 bg-card border-l border-border z-50 panel-slide panel-hidden shadow-2xl max-h-[calc(100vh-3.5rem)] h-[550px] flex flex-col overflow-hidden">
    ...
    <div class="p-4 border-b border-border bg-muted/20 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full bg-[#e53935]/15 border border-[#e53935]/30 flex items-center justify-center text-[#e53935]">
    ...
    <div id="chatbot-messages" class="flex-1 p-4 overflow-y-auto space-y-3 bg-background/50 text-xs">
        ...
        <div class="flex gap-2.5 max-w-[85%]">
            <div class="w-7 h-7 rounded-full bg-[#e53935]/10 border border-[#e53935]/20 flex items-center justify-center text-[#e53935] shrink-0">
                <i data-lucide="bot" class="w-3.5 h-3.5"></i>
            </div>
            <div class="bg-muted px-3 py-2 rounded-2xl rounded-tl-none text-foreground leading-relaxed shadow-sm">
                Halo! Saya adalah **Pemandu Chatbot**. Ada yang membingungkan dari Portal Beasiswa ini? Tanyakan saja atau klik salah satu topik di bawah!
            </div>
        </div>
    </div>
    ...
    <div class="p-3 border-t border-border bg-card shrink-0 flex gap-2">
        <input type="text" id="chatbot-input" placeholder="Ketik pesan Anda di sini..." class="flex-1 bg-muted text-foreground placeholder-muted-foreground px-4 py-2.5 rounded-xl border border-border text-xs focus:outline-none focus:ring-1 focus:ring-[#e53935]/50">
        ...
        <div class="bg-card border-2 border-border rounded-2xl shadow-2xl w-full max-w-md relative overflow-hidden z-10 modal-box">
            ...
            <div class="p-6">
                <div class="flex items-center gap-2 mb-6">
                    <i data-lucide="settings" class="w-5 h-5 text-[#e53935]"></i>
                    <h2 class="text-foreground font-black text-lg">Pengaturan Akun</h2>
                </div>
                <div class="flex flex-col items-center gap-3 mb-6">
                    <div class="relative">
                        <div id="modal-avatar" class="w-20 h-20 rounded-full border-4 border-[#e53935] shadow-[4px_4px_0px_0px_rgba(229,57,53,0.3)] overflow-hidden bg-muted flex items-center justify-center cursor-pointer">
                        ...
                        <button type="button" id="btn-upload-avatar" class="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-[#e53935] hover:bg-[#c62828] text-white flex items-center justify-center shadow-lg border border-red-700 active:scale-95 transition-transform">
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        <input type="text" id="modal-edit-name" class="w-full bg-muted text-foreground text-sm px-4 py-2.5 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-[#e53935]/50 transition-shadow" placeholder="Nama lengkap Anda">
                    </div>
                    <div>
                    ...
                    <div class="border-t border-border pt-4 mt-4">
                        ...
                        <div id="password-section" class="hidden mt-3 space-y-3">
                            <div>
                                ...
                                <div class="relative">
                                    <input type="password" id="modal-current-password" class="w-full bg-muted text-foreground text-xs px-4 py-2 pr-10 rounded-xl border border-border focus:outline-none focus:ring-1 focus:ring-[#e53935]/50" placeholder="••••••••">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="password" id="modal-new-password" class="w-full bg-muted text-foreground text-xs px-4 py-2 pr-10 rounded-xl border border-border focus:outline-none focus:ring-1 focus:ring-[#e53935]/50" placeholder="••••••••">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="password" id="modal-confirm-password" class="w-full bg-muted text-foreground text-xs px-4 py-2 pr-10 rounded-xl border border-border focus:outline-none focus:ring-1 focus:ring-[#e53935]/50" placeholder="••••••••">
